...
var Map = React.createClass({
	...
	getDefaultProps: function () {
	   return {
	     ...
	     layers: [
	     	{
				emme_links: L.tileLayer('/database/pggis/table/emme_links3857/{z}/{x}/{y}.png?cache_policy=persist&sql=select * from emme_links3857', {
				    maxZoom: 18
				}),
				emme_tlines: L.tileLayer('/database/pggis/table/emme_tlines3857/{z}/{x}/{y}.png?cache_policy=persist&sql=select * from emme_tlines3857', {
				    maxZoom: 18
				}),
				emme_nodes: L.tileLayer('/database/pggis/table/emme_nodes3857/{z}/{x}/{y}.png?cache_policy=persist&sql=select * from emme_nodes3857', {
					maxZoom: 18
				}),
				emme_spider: L.tileLayer('/database/pggis/table/emme_spider/{z}/{x}/{y}.png?cache_policy=persist&sql=select * from emme_spider', {
					maxZoom: 18
				})
	     	}
	     ],
	     dynamicLayers: [
	     	{
				emme_costliest_dynamic: L.tileLayer('/database/pggis/table/emme_costliest/{z}/{x}/{y}.png?cache_policy=persist&sql=select * from emme_costliest', {
					maxZoom: 18
				})
	     	}
	     ]
	...
	componentDidMount: function() {
		...
		// var emme_costliest_dynamic = L.tileLayer('/database/pggis/table/emme_costliest/{z}/{x}/{y}.png?cache_policy=persist&sql=SELECT DISTINCT ON(c.fid) c.*, ST_MakeLine(a.geom, b.geom) AS geom FROM emme_veh AS c, emme_nodes3857 AS b, emme_nodes3857 AS a WHERE c.fid = a.id AND c.tid = b.id AND ST_Distance(a.geom, b.geom) < 50000 AND c.cost > 1 ORDER BY c.fid, c.cost DESC', {
		// var emme_costliest_dynamic = L.tileLayer('/database/pggis/table/emme_costliest/{z}/{x}/{y}.png?cache_policy=persist&sql=SELECT DISTINCT ON(c.fid) c.*, ST_MakeLine(a.geom, b.geom) AS geom FROM emme_veh AS c, emme_nodes3857 AS b, emme_nodes3857 AS a WHERE c.fid = a.id AND c.tid = b.id AND ST_Distance(a.geom, b.geom) < '+this.props.distance+' AND c.cost > 1 ORDER BY c.fid, c.cost DESC', {
		// 	maxZoom: 18
		// }).addTo(this.map);

		var emme_nodes_grid = new L.UtfGrid('/database/pggis/table/emme_nodes3857/{z}/{x}/{y}.grid.json?cache_policy=persist&interactivity=id,inboai&sql=select geom,id,inboai::text from emme_nodes3857', {
		    useJsonP: false
		});
	...
	componentDidUpdate: function() {
		...
		if(this.props.clickedObject) {
			...
			this.props.dynamicLayers[0].emme_costliest_dynamic = L.tileLayer(
				'/database/pggis/table/emme_costliest/{z}/{x}/{y}.png?cache_policy=persist&sql='+sql, {
				maxZoom: 18
			}).addTo(window.map);
			this.props.dynamicLayers[0].emme_costliest_dynamic.bringToFront();
		}
		...
		if(this.props.distance) {
			...
			// if(this.props.dynamicLayers[0].emme_costliest_dynamic) window.map.removeLayer(this.props.dynamicLayers[0].emme_costliest_dynamic);
			// this.props.dynamicLayers[0].emme_costliest_dynamic = L.tileLayer(
			// 	'/database/pggis/table/emme_costliest/{z}/{x}/{y}.png?cache_policy=persist&sql=SELECT DISTINCT ON(c.fid) c.*, ST_MakeLine(a.geom, b.geom) AS geom FROM emme_veh AS c, emme_nodes3857 AS b, emme_nodes3857 AS a WHERE c.fid = a.id AND c.tid = b.id AND ST_Distance(a.geom, b.geom) < ' + this.props.distance.toString() + ' AND c.cost > 0 ORDER BY c.fid, c.cost DESC', {
			// 	maxZoom: 18
			// }).addTo(window.map);
			// this.props.dynamicLayers[0].emme_costliest_dynamic.bringToFront();
